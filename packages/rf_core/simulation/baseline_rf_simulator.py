...
from packages.rf_core.materials.rf_materials import MaterialProfileRegistry
from packages.rf_core.models.rf_models import (
    AccessPoint,
    ApLayout,
    Opening,
    Point2D,
...
from packages.rf_core.rules.object_obstacle_rules import (
...
def build_heatmap_image_data_url(
    scene: Scene,
    ap_layout: ApLayout,
    result: SimulationResult,
) -> str:
    ...
    for room in scene.rooms:
        ...
        ax.plot(xs, ys, linestyle="--", linewidth=1)
        if room.centroid is not None:
            ax.text(room.centroid.x, room.centroid.y, room.room_name, fontsize=8, ha="center")

    for wall in scene.walls:
        label = wall.material if not wall.is_exterior else f"{wall.material} (ext)"
        ax.plot([wall.start.x, wall.end.x], [wall.start.y, wall.end.y], linewidth=2.5)
        ax.text(
            (wall.start.x + wall.end.x) / 2.0,
            (wall.start.y + wall.end.y) / 2.0,
            label,
    ...
    for ap in ap_layout.aps:
        ax.scatter(ap.position.x, ap.position.y, marker="x", s=120, label=ap.ap_name)
        ax.text(ap.position.x + 0.1, ap.position.y + 0.1, ap.ap_name)
