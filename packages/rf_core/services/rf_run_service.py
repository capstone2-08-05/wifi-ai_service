...
`run_rf(...)` 가 DTO 수신 → (adapter) → baseline 시뮬 → 결과 반환.
"""
...
from packages.rf_core.adapters.baseline import sionna_input_dto_to_baseline_scene_and_layout
from packages.rf_core.dto.backend_scene import SionnaInputDTO
from packages.rf_core.models.rf_models import ApLayout, Scene, SimulationConfig
from packages.rf_core.persistence.rf_persistence import JsonPersistenceStore
from packages.rf_core.simulation.baseline_rf_simulator import (
    BaselineRfSimulator,
    save_outputs,
)
...
def _resolve_image_url(rf_run_id: str) -> str:
    template = os.environ.get(
        "RF_ARTIFACT_IMAGE_URL_TEMPLATE",
        "http://localhost:9000/internal/rf/images/{rf_run_id}.png",
    ).strip()
    return template.format(rf_run_id=rf_run_id)
...
def run_rf(
    *,
    ...
    """
    Baseline RF 한 번 실행 후 결과를 반환한다.
    `persist_outputs=True`일 때만 로컬 디스크에 manifest/heatmap 등을 저장한다.
    ...
    store: JsonPersistenceStore | None = None
    if persist_outputs:
    ...
    try:
        ...
        if persist_outputs:
            ...
            run_output_root_str = str(run_output_root.resolve())
        else:
            manifest = {
                "scene_version_id": scene.scene_version_id,
                "layout_name": layout.layout_name,
                "artifacts": {"imageUrl": image_url} if image_url else {},
        ...
        return RfRunResult(
            ...
            output_root=run_output_root_str,
            manifest=manifest,
            paths=path_bundle,
            error=None,
        )
