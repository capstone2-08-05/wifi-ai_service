...
class RadioMaterial(BaseModel):
    ...
    - attenuation_scale: 통과 손실 보정 계수 (1.0 = 보정 없음).
        runtime에서 ITURadioMaterial thickness 에 곱해서 transmission/reflection loss 를 조정.
        기하 mesh 와 시각화는 영향 받지 않음. **현재 적용됨.**
    - loss_offset_db: RSSI 추가 오프셋 (dB).
        per-cell material mask 인프라가 도입되면 후처리 offset 으로 적용 예정.
        값은 plan과 response 양쪽에 echo 되지만 현재 runtime은 미적용.
    - learnable: 추후 differentiable optimization 대상 여부. 현재는 metadata flag.
    """
