...
주의 — 현재 적용 범위:
- 벽(WallObject)만 Sionna `SceneObject` 로 mesh가 생성되어 ray-tracing에 참여한다.
- OpeningObject(door/window)와 FurnitureObject는 plan에 metadata로 전달되지만
  현재 runtime은 wall opening subtraction과 furniture mesh를 생성하지 않으므로
  실제 전파 시뮬레이션에는 반영되지 않는다. 후속 PR에서 처리 예정.
- RadioMaterial 의 `attenuation_scale`, `loss_offset_db`, `learnable` 필드 또한
  현재 runtime이 적용하지 않으며, RSSI calibration 단계 도입 시 사용된다.
"""
